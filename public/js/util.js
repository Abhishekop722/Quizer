function* numAutoGenerator(init){
    var counter =init;
    while(true){
        ++counter;
        yield counter;
...
function autocomplete(inp, ids,names) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < ids.length; i++) {
          ...
          if (ids[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            ...
            b.innerHTML = "<strong>" + ids[i].substr(0, val.length) + "</strong>";
            b.innerHTML += ids[i].substr(val.length);
            b.innerHTML +="(Id)";
    ...
    document.querySelector('#querySearch').addEventListener("blur", function (e) {
        closeAllLists(e.target);
    });
}
function notification(str){
  var div = document.createElement('div');
  div.className = "notification alert-info animated slideInDown";
  div.innerText = str;
